...
class CalculatorDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.input_field.installEventFilter(self) # For smart operator prepending
    ...
    def eventFilter(self, obj, event):
        if obj is self.input_field and event.type() == QEvent.KeyPress:
            # If input is selected and user types an operator (+, -, *, /), 
            # and the input field is just the previous result, we can keep it.
            # If the user starts typing a number, we might want to clear it.
            # But the simplest is: if user types an operator and field is empty, 
            # use last_result.
            if self.last_result is not None and not self.input_field.text():
                text = event.text()
                if text in ('+', '-', '*', '/', '%', '^'):
                    self.input_field.setText(str(self.last_result))
                    # Let the event propagate to add the character
        return super().eventFilter(obj, event)

    def preprocess_expression(self, expr):
        # Replace ^ with ** for power
        expr = expr.replace('^', '**')
        # Treat % as percentage: 11% -> (11*0.01)
        # We replace the % character with *0.01
        # To handle cases like (10+10)%, we replace )% with )*0.01
        expr = expr.replace(')%', ')*0.01')
        # Use regex to find numbers followed by % and replace them
