...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                               ...
                               QSpinBox, QDoubleSpinBox, QFrame, QGroupBox, QDateEdit, QAbstractSpinBox,
                               QComboBox, QStyledItemDelegate, QListWidget, QListWidgetItem, QScrollArea,
...
class ProductSearchDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["Name", "Barcode", "MRP", "Price", "Base UOM"])
    ...
    def load_products(self):
        ...
        for row, prod in enumerate(products):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(str(prod[3] or "0.0"))) # MRP
            self.table.setItem(row, 3, QTableWidgetItem(str(prod[4] or "0.0"))) # Price
            self.table.setItem(row, 4, QTableWidgetItem(str(prod[6] or "pcs"))) # Base UOM
...
class RecycleBinDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.label.setStyleSheet("font-weight: bold; color: #f38ba8;")
        layout.addWidget(self.label)
        
        self.table = QTableWidget()
        self.table.setColumnCount(4)
        self.table.setHorizontalHeaderLabels(["Name", "Barcode", "Deleted At", "Restore"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.table)
    ...
    def load_deleted_products(self):
        ...
        for row, p in enumerate(products):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(p[7].strftime("%d-%m-%Y %H:%M")))
            
            res_btn = QPushButton("Restore")
            res_btn.setStyleSheet("background-color: #a6e3a1; color: #1e1e2e;")
...
class SchemeEntryDialog(QDialog):
    def __init__(self, db_manager, scheme_id=None, parent=None):
        ...
        self.scheme_name = QLineEdit(); self.scheme_name.setPlaceholderText("Scheme Name")
        self.valid_from = QDateEdit(); self.valid_from.setDisplayFormat("dd-MM-yyyy"); self.valid_from.setDate(QDate.currentDate()); self.valid_from.setCalendarPopup(True)
        self.valid_to = QDateEdit(); self.valid_to.setDisplayFormat("dd-MM-yyyy"); self.valid_to.setDate(QDate.currentDate().addDays(365)); self.valid_to.setCalendarPopup(True)
        header_layout.addWidget(QLabel("&Name:")); header_layout.addWidget(self.scheme_name, 1)
        header_layout.addWidget(QLabel("Valid &From:")); header_layout.addWidget(self.valid_from)
        header_layout.addWidget(QLabel("&To:")); header_layout.addWidget(self.valid_to)
        layout.addWidget(header_grp)
        
        rule_grp = QGroupBox("Add Item Rule")
        rule_layout = QHBoxLayout(rule_grp)
        self.selected_rule_item = None
        self.item_btn = QPushButton("Select &Item (F3)"); self.item_btn.setShortcut("F3"); self.item_btn.clicked.connect(self.select_rule_item)
        self.item_label = QLabel("<No Item>"); self.item_label.setStyleSheet("color: #fab387; font-weight: bold;"); self.item_label.setMinimumWidth(150)
        self.min_qty = QDoubleSpinBox(); self.min_qty.setValue(1.0); self.min_qty.setPrefix("Min: "); self.min_qty.setDecimals(3)
        self.max_qty = QDoubleSpinBox(); self.max_qty.setMaximum(1000000); self.max_qty.setPrefix("Max: "); self.max_qty.setSpecialValueText("Max: ∞"); self.max_qty.setDecimals(3)
        self.target_uom = QComboBox(); self.target_uom.addItems(["<All UOMs>"] + [u[1] for u in self.db.get_uoms()])
        self.benefit_type = QComboBox(); self.benefit_type.addItems(["Percent (%)", "Flat Amt (Rs)", "Fixed Rate"])
        self.benefit_value = QDoubleSpinBox(); self.benefit_value.setMaximum(1000000); self.benefit_value.setDecimals(3)
        add_rule_btn = QPushButton("&Add"); add_rule_btn.clicked.connect(self.add_rule_to_list); add_rule_btn.setStyleSheet("background-color: #a6e3a1; color: #1e1e2e;")
        rule_layout.addWidget(self.item_btn); rule_layout.addWidget(self.item_label, 1)
        rule_layout.addWidget(self.min_qty); rule_layout.addWidget(self.max_qty)
        rule_layout.addWidget(QLabel("&UOM:")); rule_layout.addWidget(self.target_uom)
        rule_layout.addWidget(QLabel("&Type:")); rule_layout.addWidget(self.benefit_type)
        rule_layout.addWidget(QLabel("&Value:")); rule_layout.addWidget(self.benefit_value)
        rule_layout.addWidget(add_rule_btn)
        layout.addWidget(rule_grp)
        
        self.items_list = QTableWidget(); self.items_list.setColumnCount(8); self.items_list.setHorizontalHeaderLabels(["Item Name", "ID", "Min Qty", "Max Qty", "UOM", "Type", "Value", "Action"])
        self.items_list.setColumnHidden(1, True); self.items_list.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        self.items_list.setMinimumHeight(300)
        layout.addWidget(self.items_list)
        
        footer = QHBoxLayout()
        save_btn = QPushButton("&Save Scheme (F2)"); save_btn.clicked.connect(self.save_scheme); save_btn.setObjectName("btnSave")
        cancel_btn = QPushButton("&Cancel (Esc)"); cancel_btn.clicked.connect(self.close); cancel_btn.setObjectName("btnCancel")
        footer.addStretch(); footer.addWidget(save_btn); footer.addWidget(cancel_btn)
        layout.addLayout(footer)
        
        scroll.setWidget(content_widget)
        main_layout.addWidget(scroll)
        
        if self.scheme_id: self.load_scheme_data()
        save_act = QAction(self); save_act.setShortcut("F2"); save_act.triggered.connect(self.save_scheme); self.addAction(save_act)
        cancel_act = QAction(self); cancel_act.setShortcut("Esc"); cancel_act.triggered.connect(self.close); self.addAction(cancel_act)

    def select_rule_item(self):
        dlg = ProductSearchDialog(self.db, self)
        if dlg.exec() == QDialog.Accepted:
            self.selected_rule_item = dlg.selected_product
            self.item_label.setText(f"{self.selected_rule_item[1]} ({self.selected_rule_item[2]})")

    def add_rule_to_list(self):
        if not self.selected_rule_item: return
        self._add_row_to_table(self.selected_rule_item[1], self.selected_rule_item[0], self.min_qty.value(), self.max_qty.value(), 
                               self.target_uom.currentText(), self.benefit_type.currentIndex(), self.benefit_value.value())
        self.selected_rule_item = None; self.item_label.setText("<No Item>")

    def _add_row_to_table(self, pname, pid, min_q, max_q, uom, b_idx, val):
        row = self.items_list.rowCount(); self.items_list.insertRow(row)
        self.items_list.setItem(row, 0, QTableWidgetItem(pname)); self.items_list.setItem(row, 1, QTableWidgetItem(str(pid)))
        self.items_list.setItem(row, 2, QTableWidgetItem(f"{min_q:.3f}")); self.items_list.setItem(row, 3, QTableWidgetItem(f"{max_q:.3f}" if max_q > 0 else "∞"))
        self.items_list.setItem(row, 4, QTableWidgetItem(uom))
        b_type = "percent" if b_idx == 0 else "amount" if b_idx == 1 else "absolute_rate"
        self.items_list.setItem(row, 5, QTableWidgetItem(b_type)); self.items_list.setItem(row, 6, QTableWidgetItem(f"{val:.3f}"))
        del_btn = QPushButton("Del"); del_btn.setStyleSheet("background-color: #ff5555;"); del_btn.clicked.connect(lambda: self.items_list.removeRow(self.items_list.currentRow()))
        self.items_list.setCellWidget(row, 7, del_btn)

    def load_scheme_data(self):
        header = next((s for s in self.db.get_schemes() if s[0] == self.scheme_id), None)
        if header:
            self.scheme_name.setText(header[1])
            if header[2]: self.valid_from.setDate(header[2])
            if header[3]: self.valid_to.setDate(header[3])
        for r in self.db.get_scheme_rules(self.scheme_id):
            b_idx = 0 if r[6] == 'percent' else 1 if r[6] == 'amount' else 2
            self._add_row_to_table(r[1], r[0], float(r[3]), float(r[4]) if r[4] else 0, r[5] or "<All UOMs>", b_idx, float(r[7]))

    def save_scheme(self):
        name = self.scheme_name.text()
        if not name or self.items_list.rowCount() == 0: return
        items_data = []
        for r in range(self.items_list.rowCount()):
            items_data.append({'pid': int(self.items_list.item(r, 1).text()), 'min_qty': float(self.items_list.item(r, 2).text()),
                               'max_qty': float(self.items_list.item(r, 3).text()) if self.items_list.item(r, 3).text() != "∞" else None,
                               'target_uom': None if self.items_list.item(r, 4).text() == "<All UOMs>" else self.items_list.item(r, 4).text(),
                               'benefit_type': self.items_list.item(r, 5).text(), 'benefit_value': float(self.items_list.item(r, 6).text())})
        v_from, v_to = self.valid_from.date().toPython(), self.valid_to.date().toPython()
        if self.scheme_id: success = self.db.update_scheme(self.scheme_id, name, v_from, v_to, items_data)
        else: success = self.db.add_scheme(name, v_from, v_to, items_data)
        if success: 
            QMessageBox.information(self, "Success", f"Scheme '{name}' Saved Successfully.")
            self.accept()
...
class SchemeListDialog(QDialog):
    def __init__(self, db_manager, mode="list", parent=None):
        ...
        layout = QVBoxLayout(self)
        self.table = QTableWidget(); self.table.setColumnCount(5); self.table.setHorizontalHeaderLabels(["ID", "Scheme Name", "Date Range", "Included Items", "Action"])
        self.table.setColumnHidden(0, True); self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.Stretch)
        self.table.setSelectionBehavior(QAbstractItemView.SelectRows)
        if self.mode == "modify": self.table.doubleClicked.connect(self.modify_selected)
        layout.addWidget(self.table)
        self.load_schemes()
    ...
    def load_schemes(self):
        ...
        for row, s in enumerate(self.db.get_schemes()):
            self.table.insertRow(row)
            self.table.setItem(row, 0, QTableWidgetItem(str(s[0]))); self.table.setItem(row, 1, QTableWidgetItem(s[1]))
            date_range = f"{s[2].strftime('%d-%m-%Y')} to {s[3].strftime('%d-%m-%Y')}"
            self.table.setItem(row, 2, QTableWidgetItem(date_range)); self.table.setItem(row, 3, QTableWidgetItem(s[4]))
            btn = QPushButton("&Del" if self.mode == 'list' else "&Modify")
            if self.mode == 'list': btn.setObjectName("btnCancel")
            else: btn.setObjectName("btnSave")
            btn.clicked.connect(lambda _, sid=s[0]: self.delete_scheme(sid) if self.mode == 'list' else self.open_modify(sid))
            self.table.setCellWidget(row, 4, btn)

    def delete_scheme(self, sid):
        if QMessageBox.question(self, "Confirm", "Delete Scheme?") == QMessageBox.Yes:
            self.db.delete_scheme(sid); self.load_schemes()

    def modify_selected(self):
        row = self.table.currentRow()
        if row >= 0: self.open_modify(int(self.table.item(row, 0).text()))

    def open_modify(self, sid):
        res = SchemeEntryDialog(self.db, scheme_id=sid, parent=self).exec()
        self.showFullScreen()
        if res == QDialog.Accepted: self.load_schemes()
...
class UOMMasterDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.uom_input = QLineEdit(); self.uom_input.setPlaceholderText("New UOM Name")
        self.alias_input = QLineEdit(); self.alias_input.setPlaceholderText("Alias (Short)")
        self.alias_input.setFixedWidth(100)
        add_btn = QPushButton("&Add"); add_btn.clicked.connect(self.add_uom)
        input_layout.addWidget(QLabel("UOM:")); input_layout.addWidget(self.uom_input)
        input_layout.addWidget(QLabel("Alias:")); input_layout.addWidget(self.alias_input)
        input_layout.addWidget(add_btn)
        layout.addLayout(input_layout)
        self.list_widget = QTableWidget(); self.list_widget.setColumnCount(3); self.list_widget.setHorizontalHeaderLabels(["UOM Name", "Alias", "Action"])
    ...
    def add_uom(self):
        ...
        if name and self.db.add_uom(name, alias): 
            self.uom_input.clear(); self.alias_input.clear(); self.load_uoms()
            self.accept()
    ...
    def load_uoms(self):
        ...
        for row, u in enumerate(self.db.get_uoms()):
            ...
            del_btn = QPushButton("&Del"); del_btn.clicked.connect(lambda _, name=u[1]: self.delete_uom(name)); self.list_widget.setCellWidget(row, 2, del_btn)
            
    def delete_uom(self, name):
        if QMessageBox.question(self, "Confirm", f"Delete UOM '{name}'?") == QMessageBox.Yes: self.db.delete_uom(name); self.load_uoms()
...
class LanguageMasterDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        layout = QVBoxLayout(self)
        input_layout = QHBoxLayout()
        self.lang_input = QLineEdit(); self.lang_input.setPlaceholderText("Language Name (e.g. Hindi)")
        self.code_input = QLineEdit(); self.code_input.setPlaceholderText("Code (hi)")
        add_btn = QPushButton("&Add"); add_btn.clicked.connect(self.add_lang)
        input_layout.addWidget(QLabel("Lang:")); input_layout.addWidget(self.lang_input)
        input_layout.addWidget(QLabel("Code:")); input_layout.addWidget(self.code_input)
        input_layout.addWidget(add_btn)
        layout.addLayout(input_layout)
        
        self.table = QTableWidget(); self.table.setColumnCount(4); self.table.setHorizontalHeaderLabels(["Name", "Code", "Translations", "Action"])
    ...
    def add_lang(self):
        ...
        if name and self.db.add_language(name, code):
            self.lang_input.clear(); self.code_input.clear(); self.load_langs()
            self.accept()
    ...
    def load_langs(self):
        ...
        for row, l in enumerate(self.db.get_languages()):
            ...
            trans_btn.setStyleSheet("background-color: #89b4fa; color: #1e1e2e;")
            trans_btn.clicked.connect(lambda _, lid=l[0], lname=l[1]: self.open_translations(lid, lname))
    ...
    def delete_lang(self, lid):
        if QMessageBox.question(self, "Confirm", "Delete Language?") == QMessageBox.Yes:
            self.db.delete_language(lid); self.load_langs()

    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape: self.close()
        else: super().keyPressEvent(event)

__all__ = ['MainWindow', 'ConfigDialog', 'LoginDialog', 'SuperUserCreationDialog', 'MODERN_STYLE']
...
class LoginDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        if not pixmap.isNull():
            self.splash_label.setPixmap(pixmap.scaled(350, 350, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            self.splash_label.setAlignment(Qt.AlignCenter)
            layout.addWidget(self.splash_label)
        
        title = QLabel("System Login")
        title.setStyleSheet("font-size: 20pt; font-weight: bold; color: #89b4fa; margin-bottom: 10px;")
        ...
        copyright_label.setStyleSheet("font-size: 8pt; color: #585b70; margin-top: 15px;")
    ...
    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Return, Qt.Key_Enter): self.login()
        else: super().keyPressEvent(event)
...
class SuperUserCreationDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        title.setStyleSheet("font-size: 18pt; font-weight: bold; color: #a6e3a1; margin-bottom: 10px;")
        layout.addWidget(title, 0, Qt.AlignCenter)
        
        info = QLabel("No users found in database.\nPlease create the first Super Administrator.")
        info.setStyleSheet("color: #fab387; margin-bottom: 15px;")
    ...
    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Return, Qt.Key_Enter): self.create_user()
        else: super().keyPressEvent(event)
...
class UserMasterDialog(QDialog):
    ...
    def load_users(self):
        ...
        for row, u in enumerate(self.db.get_users()):
            # u: (id, username, full_name, is_superuser)
            self.table.insertRow(row)
    ...
    def delete_user(self, uid):
        if QMessageBox.question(self, "Confirm", "Delete User?") == QMessageBox.Yes:
            self.db.delete_user(uid); self.load_users()
...
class CustomerMasterDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.master_search_input.setPlaceholderText("Search Customer by Name or Mobile...")
        ...
        self.table = QTableWidget(); self.table.setColumnCount(5); self.table.setHorizontalHeaderLabels(["Name", "Mobile", "Address", "Email", "Action"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.table)
        self.load_customers()
    ...
    def add_customer(self):
        if not self.name.text() or not self.mobile.text():
            ...
            return
        if self.db.add_customer(self.name.text(), self.mobile.text(), self.address.text(), self.email.text()):
            self.name.clear(); self.mobile.clear(); self.address.clear(); self.email.clear()
            self.load_customers()
            self.accept()
    ...
    def load_customers(self):
        ...
        customers = self.db.search_customers(query_text) if query_text else self.db.get_customers()
        for row, c in enumerate(customers):
            ...
            del_btn = QPushButton("Del")
            del_btn.clicked.connect(lambda _, cid=c[0]: self.db.delete_customer(cid) or self.load_customers())
...
class CustomerSearchDialog(QDialog):
    ...
    def load_customers(self):
        query = self.search_input.text()
        customers = self.db.search_customers(query) if query else self.db.get_customers()
        self.table.setRowCount(0)
        for row, c in enumerate(customers):
            self.table.insertRow(row)
            self.table.setItem(row, 0, QTableWidgetItem(c[1]))
    ...
    def keyPressEvent(self, event):
        ...
        elif event.key() in (Qt.Key_Return, Qt.Key_Enter): self.select_customer()
        else: super().keyPressEvent(event)
...
class PurchaseRegisterDialog(QDialog):
    def __init__(self, db_manager, product_id, product_name, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["Date", "Supplier", "Inv No", "Qty", "Rate", "UOM", "MRP"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.table)
    ...
    def load_register(self, product_id):
        ...
        for row, r in enumerate(rows):
            # r: (timestamp, supplier_name, invoice_no, quantity, purchase_rate, uom, mrp)
            self.table.insertRow(row)
            ...
            self.table.setItem(row, 6, QTableWidgetItem(f"{float(r[6]):.2f}" if r[6] else "0.00"))
...
class PurchaseEntryDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.item_search = QLineEdit(); self.item_search.setPlaceholderText("Enter Item Name to find its purchases...")
        self.item_search.textChanged.connect(self.load_search_results)
        s_layout.addWidget(QLabel("Item:")); s_layout.addWidget(self.item_search)
        layout.addWidget(search_grp)
        
        self.search_table = QTableWidget(); self.search_table.setColumnCount(5); self.search_table.setHorizontalHeaderLabels(["ID", "Date", "Supplier", "Invoice", "Total"])
        self.search_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); self.search_table.setFixedHeight(150)
        ...
        self.invoice_input = QLineEdit(); self.invoice_input.setPlaceholderText("Invoice No")
        header.addWidget(QLabel("Supplier:")); header.addWidget(self.supplier_input, 1)
        header.addWidget(QLabel("Inv No:")); header.addWidget(self.invoice_input)
        layout.addLayout(header)
        
        self.table = QTableWidget()
        self.table.setColumnCount(6)
        self.table.setHorizontalHeaderLabels(["Barcode/Name", "Item Name", "Qty", "UOM", "Purchase Rate", "MRP"])
        self.table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.table.setRowCount(1)
        self.table.setItemDelegateForColumn(0, FuzzyCompleterDelegate(self.db, self.table))
        ...
        self.lbl_total.setStyleSheet("font-size: 18pt; font-weight: bold; color: #a6e3a1;")
        ...
        footer.addWidget(save_btn); footer.addStretch(); footer.addWidget(self.lbl_total); footer.addSpacing(20); footer.addWidget(close_btn)
        main_layout.addLayout(footer)
        
        self.table.setFocus(); self.table.setCurrentCell(0, 0)
    ...
    def load_search_results(self):
        ...
        for row, r in enumerate(results):
            ...
            self.search_table.setItem(row, 1, QTableWidgetItem(r[1].strftime("%d-%m-%Y")))
        ...
        try:
            ...
            if col == 0:
                ...
                if barcode:
                    ...
                    if product:
                        self.table.item(row, 0).setText(product[2]) # Fill with standardized barcode
                        ...
                        self.table.setItem(row, 4, QTableWidgetItem(f"{product[4]:.2f}"))
                        self.table.setItem(row, 5, QTableWidgetItem(f"{product[3]:.2f}"))
                        if row == self.table.rowCount() - 1:
                            self.table.setRowCount(row + 2)
                        
                        # Move focus to Qty column
                        QTimer.singleShot(0, lambda: self.table.setCurrentCell(row, 2))
            self.recalc_total()
        finally: self.updating_cell = False
    ...
    def save_purchase(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            if not name_item or not name_item.data(Qt.UserRole): continue
            try:
                ...
                if qty > 0:
                    items.append({'pid': pid, 'qty': qty, 'rate': rate, 'uom': uom, 'mrp': mrp_val})
                    total += qty * rate
            except: continue
            
        if not items: return
        if self.db.record_purchase(self.supplier_input.currentText(), self.invoice_input.text(), items, total):
            QMessageBox.information(self, "Success", "Purchase recorded successfully.")
            self.accept()

    def keyPressEvent(self, event):
        if event.key() == Qt.Key_F2: self.save_purchase()
        elif event.key() == Qt.Key_Escape: self.close()
        elif event.key() == Qt.Key_F3: self.open_search_dialog()
        else: super().keyPressEvent(event)
    ...
    def open_search_dialog(self):
        ...
        if dlg.exec() == QDialog.Accepted:
            row = self.table.currentRow()
            if row < 0: row = self.table.rowCount() - 1
            if self.table.item(row, 0) and self.table.item(row, 0).text():
                if row == self.table.rowCount() - 1: self.table.setRowCount(row + 2)
                row += 1; self.table.setCurrentCell(row, 0)
...
class HeldSalesDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.table = QTableWidget(); self.table.setColumnCount(5); self.table.setHorizontalHeaderLabels(["ID", "Time", "Amount", "User", "Action"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table.setSelectionBehavior(QAbstractItemView.SelectRows)
        ...
        close_btn = QPushButton("&Close (Esc)")
        close_btn.clicked.connect(self.close)
        btn_layout.addWidget(restore_btn); btn_layout.addWidget(close_btn)
    ...
    def load_held_sales(self):
        ...
        for row, s in enumerate(sales):
            ...
            del_btn = QPushButton("Del")
            del_btn.clicked.connect(lambda _, hid=s[0]: self.db.delete_held_sale(hid) or self.load_held_sales())
            self.table.setCellWidget(row, 4, del_btn)
    ...
    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Return, Qt.Key_Enter): self.select_bill()
...
class TranslationManagerDialog(QDialog):
    ...
    def load_items(self):
        ...
        # We need a db method to get all translations for a language
        # For now we'll fetch per item or use a map
        self.table.setRowCount(0)
        for row, p in enumerate(products):
            # p: (id, name, barcode, mrp, price, category, base_uom)
            self.table.insertRow(row)
            ...
            save_btn.clicked.connect(lambda _, pid=p[0], edit=trans_edit: self.save_trans(pid, edit))
            self.table.setCellWidget(row, 2, save_btn)

    def save_trans(self, pid, edit):
        if self.db.add_translation(pid, self.lang_id, edit.text()):
            # Maybe show small feedback
            pass
...
class LanguageSelectionDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        btn_layout.addWidget(print_btn); btn_layout.addWidget(cancel_btn)
        layout.addLayout(btn_layout)
    ...
    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Return, Qt.Key_Enter): self.accept()
        elif event.key() == Qt.Key_Escape: self.reject()
        else: super().keyPressEvent(event)
...
class MaintenanceDashboardDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Database Maintenance Dashboard"))
    ...
    def reindex_db(self):
        if QMessageBox.question(self, "Confirm", "Reindex entire database? Application might hang for a moment.") == QMessageBox.Yes:
    ...
    def backup_db(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save Database Backup", "elytpos_backup.sql", "SQL Files (*.sql)")
        if not path: return
        ...
        cmd = ["pg_dump", "-h", params['host'], "-p", params['port'], "-U", params['user'], "-f", path, params['dbname']]
    ...
    def restore_db(self):
        ...
        if QMessageBox.question(self, "Confirm Restore", msg) != QMessageBox.Yes: return
        
        path, _ = QFileDialog.getOpenFileName(self, "Open Database Backup", "", "SQL Files (*.sql)")
        if not path: return
        ...
        cmd = ["psql", "-h", params['host'], "-p", params['port'], "-U", params['user'], "-d", params['dbname'], "-f", path]
...
class InventoryDialog(QDialog):
    def __init__(self, db_manager, parent=None):
        ...
        self.uom_display_list = [f"{u[1]} ({u[2]})" if u[2] else u[1] for u in self.uom_data_list]
        ...
        search_btn = QPushButton("&Find Item (F3)"); search_btn.clicked.connect(self.open_search)
        clear_btn = QPushButton("&New Item"); clear_btn.clicked.connect(self.clear_form)
        self.reg_btn = QPushButton("&Purchase Register"); self.reg_btn.clicked.connect(self.open_purchase_register); self.reg_btn.setVisible(False)
        top_layout.addWidget(search_btn); top_layout.addWidget(clear_btn); top_layout.addWidget(self.reg_btn); top_layout.addStretch(); layout.addLayout(top_layout)
        
        grp = QGroupBox("Base Item Details")
        form_layout = QFormLayout(grp)
        self.name_input = QLineEdit(); self.barcode_input = QLineEdit()
        self.mrp_input = QDoubleSpinBox(); self.mrp_input.setMaximum(100000); self.mrp_input.setButtonSymbols(QAbstractSpinBox.NoButtons); self.mrp_input.setDecimals(3)
        self.price_input = QDoubleSpinBox(); self.price_input.setMaximum(100000); self.price_input.setButtonSymbols(QAbstractSpinBox.NoButtons); self.price_input.setDecimals(4)
        self.category_input = QLineEdit()
        self.base_uom_input = QComboBox(); self.base_uom_input.addItems(self.uom_display_list); self.base_uom_input.setEditable(True)
        form_layout.addRow("Item &Name:", self.name_input); form_layout.addRow("Base &Barcode:", self.barcode_input); form_layout.addRow("&MRP:", self.mrp_input); form_layout.addRow("Base &Rate:", self.price_input)
        form_layout.addRow("Base &UOM:", self.base_uom_input); form_layout.addRow("&Category:", self.category_input)
        layout.addWidget(grp)

        self.alias_grp = QGroupBox("Alternate Units / Aliases"); self.alias_grp.setEnabled(False)
        alias_layout = QVBoxLayout(self.alias_grp); add_alias_layout = QHBoxLayout()
        self.alias_barcode = QLineEdit(); self.alias_uom = QComboBox(); self.alias_uom.addItems(self.uom_display_list); self.alias_uom.setEditable(True)
        self.alias_mrp = QDoubleSpinBox(); self.alias_mrp.setMaximum(1000000); self.alias_mrp.setDecimals(3); self.alias_mrp.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.alias_price = QDoubleSpinBox(); self.alias_price.setMaximum(1000000); self.alias_price.setDecimals(4); self.alias_price.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.alias_factor = QDoubleSpinBox(); self.alias_factor.setMaximum(1000000); self.alias_factor.setDecimals(3); self.alias_factor.setValue(1.0); self.alias_factor.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.alias_qty = QDoubleSpinBox(); self.alias_qty.setMaximum(1000000); self.alias_qty.setDecimals(3); self.alias_qty.setValue(1.0); self.alias_qty.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.alias_factor.valueChanged.connect(self.auto_calc_alias_price); self.price_input.valueChanged.connect(self.auto_calc_alias_price)
        self.alias_factor.valueChanged.connect(self.auto_calc_alias_mrp); self.mrp_input.valueChanged.connect(self.auto_calc_alias_mrp)
        add_alias_btn = QPushButton("Add &Alias"); add_alias_btn.clicked.connect(self.add_alias)
        add_alias_layout.addWidget(QLabel("Barcode:")); add_alias_layout.addWidget(self.alias_barcode)
        add_alias_layout.addWidget(QLabel("UOM:")); add_alias_layout.addWidget(self.alias_uom)
        add_alias_layout.addWidget(QLabel("MRP:")); add_alias_layout.addWidget(self.alias_mrp)
        add_alias_layout.addWidget(QLabel("Rate:")); add_alias_layout.addWidget(self.alias_price)
        add_alias_layout.addWidget(QLabel("Factor:")); add_alias_layout.addWidget(self.alias_factor)
        add_alias_layout.addWidget(QLabel("Qty:")); add_alias_layout.addWidget(self.alias_qty); add_alias_layout.addWidget(add_alias_btn)
        alias_layout.addLayout(add_alias_layout)
        self.alias_table = QTableWidget(); self.alias_table.setColumnCount(7); self.alias_table.setHorizontalHeaderLabels(["Barcode", "UOM", "MRP", "Rate", "Factor", "Qty", "Action"])
        self.alias_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); self.alias_table.setMinimumHeight(200); alias_layout.addWidget(self.alias_table); layout.addWidget(self.alias_grp)
        
        scroll.setWidget(content_widget)
        main_layout.addWidget(scroll)

        btn_layout = QHBoxLayout(); self.save_btn = QPushButton("&Save Base Item (F2)"); self.save_btn.clicked.connect(self.save_product); self.save_btn.setObjectName("btnSave")
        self.del_btn = QPushButton("&Delete Item"); self.del_btn.clicked.connect(self.confirm_delete); self.del_btn.setObjectName("btnCancel"); self.del_btn.setVisible(False)
        close_btn = QPushButton("&Close (Esc)"); close_btn.clicked.connect(self.close); close_btn.setObjectName("btnCancel")
        btn_layout.addWidget(self.del_btn); btn_layout.addStretch(); btn_layout.addWidget(self.save_btn); btn_layout.addWidget(close_btn)
        main_layout.addLayout(btn_layout)
        
        save_act = QAction(self); save_act.setShortcut("F2"); save_act.triggered.connect(self.save_product); self.addAction(save_act)
        find_act = QAction(self); find_act.setShortcut("F3"); find_act.triggered.connect(self.open_search); self.addAction(find_act)

    def keyPressEvent(self, event):
        if event.key() == Qt.Key_F2: self.save_product()
        elif event.key() == Qt.Key_Escape: self.close()
        else: super().keyPressEvent(event)
    ...
    def open_search(self):
        ...
        res = dlg.exec()
        self.showFullScreen()
        if res == QDialog.Accepted: self.load_product(dlg.selected_product)

    def load_product(self, prod):
        self.current_product_id = prod[0]; self.name_input.setText(prod[1]); self.barcode_input.setText(prod[2]); self.mrp_input.setValue(float(prod[3])); self.price_input.setValue(float(prod[4]))
        self.category_input.setText(prod[5] or ""); self.base_uom_input.setCurrentText(prod[6] or "pcs")
        self.save_btn.setText("Update Base Item (F2)"); self.alias_grp.setEnabled(True); self.load_aliases()
        self.del_btn.setVisible(True)
        self.reg_btn.setVisible(True)

    def open_purchase_register(self):
        if self.current_product_id:
            PurchaseRegisterDialog(self.db, self.current_product_id, self.name_input.text(), self).exec()
            self.showFullScreen()

    def confirm_delete(self):
        if not self.current_product_id: return
        msg = f"Are you sure you want to move '{self.name_input.text()}' to Recycle Bin?"
    ...
    def load_aliases(self):
        ...
        for row, alias in enumerate(self.db.get_aliases(self.current_product_id)):
            ...
            del_btn = QPushButton("Del"); del_btn.clicked.connect(lambda _, aid=alias[0]: self.db.delete_alias(aid) or self.load_aliases())
            self.alias_table.setCellWidget(row, 6, del_btn)

    def auto_calc_alias_price(self): self.alias_price.setValue(self.price_input.value() * self.alias_factor.value())
    def auto_calc_alias_mrp(self): self.alias_mrp.setValue(self.mrp_input.value() * self.alias_factor.value())

    def add_alias(self):
        full_uom = self.alias_uom.currentText()
        uom_name = full_uom.split(' (')[0] if ' (' in full_uom else full_uom
        
        if self.db.add_alias(self.current_product_id, self.alias_barcode.text(), uom_name, self.alias_mrp.value(), self.alias_price.value(), self.alias_factor.value(), self.alias_qty.value()):
            self.alias_barcode.clear(); self.load_aliases()

    def clear_form(self):
        self.current_product_id = None; self.name_input.clear(); self.barcode_input.clear(); self.mrp_input.setValue(0); self.price_input.setValue(0)
        self.category_input.clear(); self.alias_table.setRowCount(0); self.alias_grp.setEnabled(False); self.save_btn.setText("Save Base Item (F2)")
        self.del_btn.setVisible(False)
        self.reg_btn.setVisible(False)

    def save_product(self):
        name, barcode, mrp, price, cat = self.name_input.text(), self.barcode_input.text(), self.mrp_input.value(), self.price_input.value(), self.category_input.text() or "General"
        
        full_uom = self.base_uom_input.currentText()
        uom = full_uom.split(' (')[0] if ' (' in full_uom else full_uom
        
        if not name or not barcode: return
        if self.current_product_id:
            if self.db.update_product(self.current_product_id, name, barcode, mrp, price, cat, uom):
                QMessageBox.information(self, "Success", f"Item '{name}' Updated.")
                self.accept()
        else:
            pid = self.db.add_product(name, barcode, mrp, price, cat, uom)
            if pid: 
                self.current_product_id = pid; self.alias_grp.setEnabled(True); self.save_btn.setText("Update Base Item (F2)")
...
class SalesHistoryDialog(QDialog):
    def __init__(self, db_manager, printer, parent=None):
        ...
        layout = QVBoxLayout(self); top_layout = QHBoxLayout()
        self.date_filter = QDateEdit(); self.date_filter.setDisplayFormat("dd-MM-yyyy"); self.date_filter.setDate(QDate.currentDate()); self.date_filter.setCalendarPopup(True)
        ...
        refresh_btn = QPushButton("&Refresh"); refresh_btn.clicked.connect(self.load_history)
        top_layout.addWidget(QLabel("Date:")); top_layout.addWidget(self.date_filter)
        top_layout.addWidget(QLabel("Search:")); top_layout.addWidget(self.search_input, 1)
        top_layout.addWidget(refresh_btn); layout.addLayout(top_layout)
        
        self.table = QTableWidget(); self.table.setColumnCount(7); self.table.setHorizontalHeaderLabels(["Bill No", "Time", "Customer", "Mobile", "Amount", "Print", "Edit"])
        self.table.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); layout.addWidget(self.table); self.load_history()
    ...
    def load_history(self):
        ...
        for row, sale in enumerate(self.db.get_sales_history(self.date_filter.date().toPython(), query)):
            self.table.insertRow(row)
            ...
            p_btn = QPushButton("Print"); p_btn.clicked.connect(lambda _, sid=sale[0], amt=sale[2]: self.reprint_bill(sid, amt))
            m_btn = QPushButton("Modify"); m_btn.setObjectName("btnSave"); m_btn.clicked.connect(lambda _, sid=sale[0]: self.modify_bill(sid))
            self.table.setCellWidget(row, 5, p_btn); self.table.setCellWidget(row, 6, m_btn)
    ...
    def reprint_bill(self, sid, total):
        ...
        if items:
            ...
            if lang_dlg.exec() == QDialog.Accepted:
                print_items = self.db.get_translated_items(items, lang_dlg.selected_lang_id)
                ...
                if sale_header and sale_header[5]: 
                    ...
                    if customer:
                        cust_info = {'name': customer[1], 'mobile': customer[2], 'address': customer[3]}

                self.printer.print_receipt(print_items, float(total), sid, customer_info=cust_info)

    def modify_bill(self, sid):
        if self.parent_window: self.parent_window.load_bill_for_modification(sid); self.close()

class ExcelTable(QTableWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setColumnCount(8); self.setHorizontalHeaderLabels(["EAN/Barcode", "Item Name", "Qty", "UOM", "MRP", "Rate", "Disc %", "Amount"])
...
class FuzzySearchLineEdit(QLineEdit):
    def __init__(self, db, parent=None):
        ...
        self.popup.setStyleSheet(MODERN_STYLE + """
            QListWidget { 
                background-color: #1e1e2e; 
    ...
    def on_text_changed(self, text):
        ...
        try:
            ...
            for p in products[:10]:
                # p: (id, name, barcode, mrp, price, category, uom)
...
class MainWindow(QMainWindow):
    def __init__(self, db_manager, user):
        ...
        self.showFullScreen()
        self.db = db_manager; self.printer = ReceiptPrinter()
        self.current_user = user
        self.updating_cell = False; self.current_sale_id = None
        self.init_ui(); self.setStyleSheet(MODERN_STYLE)
    ...
    def backup_on_exit(self):
        ...
        try:
            ...
            # We don't show message box on exit to avoid blocking, maybe just a log or print
    ...
    def init_ui(self):
        ...
        if self.current_user[3]: # is_superuser
        ...
        self.date_edit = QDateEdit(); self.date_edit.setDisplayFormat("dd-MM-yyyy"); self.date_edit.setDate(QDate.currentDate()); self.date_edit.setCalendarPopup(True)
        self.bill_no_label = QLabel("Bill No: <New>")
        s_layout.addWidget(QLabel("Date:")); s_layout.addWidget(self.date_edit)
        ...
        self.cust_mobile_input = QLineEdit(); self.cust_mobile_input.setPlaceholderText("Mobile No (Lookup)")
        ...
        c_layout.addWidget(QLabel("Lookup:")); c_layout.addWidget(self.cust_mobile_input)
        c_layout.addWidget(self.cust_search_btn)
        c_layout.addWidget(self.cust_name_label); c_layout.addWidget(self.cust_mobile_label)
        c_layout.addStretch()
        ...
        btn_f4 = QPushButton("C&lear (F4)"); btn_f4.clicked.connect(self.reset_grid)
        ...
        layout.addLayout(footer)
        self.grid.setFocus(); self.grid.setCurrentCell(0, 0)
        s_shortcut = QAction(self); s_shortcut.setShortcut("F3"); s_shortcut.triggered.connect(self.open_search_dialog); self.addAction(s_shortcut)
        h_shortcut = QAction(self); h_shortcut.setShortcut("F5"); h_shortcut.triggered.connect(self.view_history); self.addAction(h_shortcut)
        hold_shortcut = QAction(self); hold_shortcut.setShortcut("F6"); hold_shortcut.triggered.connect(self.hold_current_bill); self.addAction(hold_shortcut)
        recall_shortcut = QAction(self); recall_shortcut.setShortcut("F7"); recall_shortcut.triggered.connect(self.recall_held_bill); self.addAction(recall_shortcut)
        save_shortcut = QAction(self); save_shortcut.setShortcut("F2"); save_shortcut.triggered.connect(self.process_checkout); self.addAction(save_shortcut)
        clear_shortcut = QAction(self); clear_shortcut.setShortcut("F4"); clear_shortcut.triggered.connect(self.reset_grid); self.addAction(clear_shortcut)
        quit_shortcut = QAction(self); quit_shortcut.setShortcut("Esc"); quit_shortcut.triggered.connect(self.close); self.addAction(quit_shortcut)
    ...
    def open_scheme_list(self, mode): SchemeListDialog(self.db, mode, self).exec(); self.showFullScreen()
    def open_customer_master(self): CustomerMasterDialog(self.db, self).exec(); self.showFullScreen()
    def open_customer_search(self):
        ...
        self.showFullScreen()
    def open_purchase_master(self): PurchaseEntryDialog(self.db, self).exec(); self.showFullScreen()
    def open_uom_master(self): UOMMasterDialog(self.db, self).exec(); self.showFullScreen()
    def open_language_master(self): LanguageMasterDialog(self.db, self).exec(); self.showFullScreen()
    def open_user_master(self): UserMasterDialog(self.db, self).exec(); self.showFullScreen()
    def open_maintenance(self):
        pwd, ok = QInputDialog.getText(self, "Admin Access", "Enter Maintenance Password:", QLineEdit.Password)
    ...
    def hold_current_bill(self):
        ...
        for r in range(self.grid.rowCount()):
            ...
            if not name_it or not name_it.data(Qt.UserRole): continue
            try:
                qty, rate, disc = float(self.grid.item(r, 2).text()), float(self.grid.item(r, 5).text()), float(self.grid.item(r, 6).text())
                
                mrp = 0.0
                mrp_combo = self.grid.cellWidget(r, 4)
                if mrp_combo: mrp = float(mrp_combo.currentText())
                
                eff_p = rate * (1 - disc/100)
                if qty > 0:
                    items.append({'id': name_it.data(Qt.UserRole)[0], 'name': name_it.data(Qt.UserRole)[1], 'barcode': name_it.data(Qt.UserRole)[2], 'price': eff_p, 'mrp': mrp, 'quantity': qty, 'uom': self.grid.item(r, 3).text()})
                    total += qty * eff_p
            except: continue
        if not items: return
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            for row, item in enumerate(items):
                ...
                if prod:
                    ...
                    self.grid.setItem(row, 7, QTableWidgetItem(f"{item['quantity'] * item['price']:.2f}"))
    ...
    def load_bill_for_modification(self, sid):
        self.reset_grid(); self.current_sale_id = sid; self.bill_no_label.setText(f"Bill No: {sid} [EDIT MODE]"); self.bill_no_label.setStyleSheet("color: #fab387; font-weight: bold;")
        
        # Load customer info for this bill
        ...
        items = self.db.get_sale_items(sid); self.grid.setRowCount(len(items) + 1); self.updating_cell = True
        for row, item in enumerate(items):
            prod = self.db.find_product_by_barcode(item['barcode'])
            if prod:
                self.grid.setItem(row, 0, QTableWidgetItem(item['barcode'])); self.grid.setItem(row, 1, QTableWidgetItem(item['name']))
                self.grid.setItem(row, 2, QTableWidgetItem(str(item['quantity']))); self.grid.setItem(row, 3, QTableWidgetItem(item['uom']))
                
                # Try to get MRP for the specific UOM used in the sale
                ...
                self.grid.setItem(row, 7, QTableWidgetItem(f"{item['quantity'] * calc_rate:.2f}"))
                self.grid.item(row, 1).setData(Qt.UserRole, prod)
        self.updating_cell = False; self.recalc_totals(); self.showFullScreen()

    def open_search_dialog(self):
        dlg = ProductSearchDialog(self.db, self); res = dlg.exec(); self.showFullScreen()
        if res == QDialog.Accepted:
            row = max(0, self.grid.currentRow())
            if self.grid.item(row, 0) and self.grid.item(row, 0).text():
                if row == self.grid.rowCount() - 1: self.grid.setRowCount(row + 2)
                row += 1; self.grid.setCurrentCell(row, 0)
            ...
            if col == 0:
                ...
                if barcode:
                    ...
                    if product:
                        ...
                        # Safely move focus to Qty column
                        QTimer.singleShot(0, lambda: self.grid.setCurrentCell(row, 2))
                    else:
                        for c in range(1, 7): self.grid.setItem(row, c, QTableWidgetItem(""))
            elif col == 3: 
                ...
                self.recalc_row(row)
            elif col in (2, 4, 5, 6): self.recalc_row(row)
            self.recalc_totals()
        finally: self.updating_cell = False
    ...
    def update_mrp_dropdown(self, row, product_id, uom, current_mrp):
        combo = QComboBox()
        combo.setStyleSheet("background-color: #313244; border: none; padding: 0px;")
    ...
    def recalc_row(self, row):
        try:
            qty_item, uom_item, rate_item, disc_item = self.grid.item(row, 2), self.grid.item(row, 3), self.grid.item(row, 5), self.grid.item(row, 6)
            qty, uom, rate = float(qty_item.text()) if qty_item else 0.0, uom_item.text() if uom_item else None, float(rate_item.text()) if rate_item else 0.0
            
            name_item = self.grid.item(row, 1)
            if name_item and name_item.data(Qt.UserRole):
                p_data = list(name_item.data(Qt.UserRole))
                
                # Update rate and MRP if UOM changed
                if uom and uom != p_data[5]:
                    uom_data = self.db.get_product_uom_data(p_data[0], uom)
                    if uom_data:
                        rate = uom_data['price']
                        mrp = uom_data['mrp']
                        p_data[5], p_data[7], p_data[4], p_data[3] = uom_data['uom'], uom_data['factor'], uom_data['price'], uom_data['mrp']
                        
                        # Update MRP Dropdown for new UOM
                        self.updating_cell = True
                        try:
                ...
                gross, disc_amt, cur_disc = qty * effective_rate, 0.0, float(disc_item.text()) if disc_item else 0.0
                scheme = self.db.get_active_scheme_for_product(p_data[0], qty, uom)
                if scheme:
                    s_val, s_type, s_uom = float(scheme[1]), scheme[2], scheme[3]
                    
                    # Auto-switch UOM if scheme has a specific target UOM
                    if s_uom and uom != s_uom:
                        uom = s_uom
                        self.grid.setItem(row, 3, QTableWidgetItem(uom))
                        # Refresh data for the new UOM to ensure factor is correct
                        uom_data = self.db.get_product_uom_data(p_data[0], uom)
                        if uom_data:
                            p_data[6], p_data[8], p_data[4], p_data[3] = uom_data['uom'], uom_data['factor'], uom_data['price'], uom_data['mrp']
                            name_item.setData(Qt.UserRole, tuple(p_data))
                    
                    if s_type == 'absolute_rate':
                        abs_rate = s_val
                        if is_gram: abs_rate /= 1000.0
                        gross = qty * abs_rate
                        ...
                        benefit = s_val
                        if is_gram: benefit /= 1000.0
                        disc_amt = qty * benefit
                        self.grid.setItem(row, 6, QTableWidgetItem(f"{(disc_amt / gross) * 100 if gross > 0 else 0:.3f}"))
                else: 
                    disc_amt = 0.0
                    self.grid.setItem(row, 6, QTableWidgetItem("0.0"))
    ...
    def _fmt(self, val):
        if float(val) == int(float(val)): return str(int(float(val)))
    ...
    def recalc_totals(self):
        ...
        for r in range(self.grid.rowCount()):
            ...
            try: t_qty += float(q.text())
            except: pass
            try: t_amt += float(a.text())
            except: pass
        rounded_total = round(t_amt)
        self.lbl_total_qty.setText(f"Qty: {self._fmt(t_qty)}"); self.lbl_total_amt.setText(f"Total: {self._fmt(rounded_total)}")

    def reset_grid(self):
        self.current_sale_id = None; self.bill_no_label.setText("Bill No: <New>"); self.bill_no_label.setStyleSheet("color: #ffffff;")
        self.selected_customer_data = None
        self.cust_name_label.setText("Name: <Cash>")
        self.cust_mobile_label.setText("Mob: -")
        self.grid.setRowCount(0); self.grid.setRowCount(1); self.recalc_totals(); self.grid.setFocus(); self.grid.setCurrentCell(0, 0)
    ...
    def process_checkout(self):
        ...
        if QMessageBox.question(self, "Save Voucher", msg, QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
            res = self.db.update_sale(self.current_sale_id, items, total, customer_id=cid) if self.current_sale_id else self.db.process_sale(items, total, customer_id=cid)
            if res:
                if QMessageBox.question(self, "Print", "Print Receipt?", QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
                    lang_dlg = LanguageSelectionDialog(self.db, self)
                    if lang_dlg.exec() == QDialog.Accepted:
                        print_items = self.db.get_translated_items(items, lang_dlg.selected_lang_id)
